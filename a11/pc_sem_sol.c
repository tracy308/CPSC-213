...
uthread_sem_t lowerBound;
uthread_sem_t upperBound;
uthread_sem_t mutex;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(upperBound);
    uthread_sem_wait(mutex);
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(lowerBound);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(lowerBound);
    uthread_sem_wait(mutex);
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(upperBound);
    uthread_sem_signal(mutex);  
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  // init the thread system and semaphores
  uthread_init (NUM_PROCESSORS);
  lowerBound = uthread_sem_create(0);
  upperBound = uthread_sem_create(MAX_ITEMS);
