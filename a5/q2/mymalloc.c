...
void *mymalloc(int size) {
  ...
  /* TODO: Walk through your free list to see if there is a freed block big enough
   ...
   struct mData* ptr = head;
   struct mData* prev = NULL;
   ...
   while (ptr != NULL) {
      if (size <= ptr->size) {
         ...
         return (void*)ptr + sizeof(struct mData);
      }
      prev = ptr;
      ptr = ptr->next;
  }
  ...
  if (size < 0 || size > heapsize || heapsize - size < top) {
    ...
    void *res = &heap[top];
    struct mData* data = (struct mData*) res;
    data->size = size;
    top += size;
    return res + sizeof(struct mData);
  }
}
...
void myfree(void *ptr) {
  /* TODO: Our dumb implementation does not allow for any freeing. Implement me! */
   struct mData* free = ptr - sizeof(struct mData);
   free->next = head;
   head = free;
}
